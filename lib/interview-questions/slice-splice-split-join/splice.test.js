...
Write a function named stepActions that takes in the recipe and extracts the action verbs from the steps. Fortunate for you, the action verbs are the first word of each action.
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(action => {
    result.push(action.split(' ').shift());
  });
  return result;
};
...
const removeVowels = (str) => {
  for (let i = 0; i < str.length; i++) {
    if(str[i] === 'a' || str[i] === 'e' || str[i] === 'i' || str[i] === 'o' || str[i] === 'u') {
      str.slice(str[i], str[i + 1]);
      console.log(str[i], ' I am a vowel');
    }
  }
  console.log(str);
};
